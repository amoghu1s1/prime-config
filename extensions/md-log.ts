...
export default function mdLog(pi: ExtensionAPI) {
	...
	// The teach flow creates vault/learn/<topic>.md first, then writes
	// ~/.prime/agent/md-log-config.json pointing at it — DURING the session.
	// We record the config fingerprint at session start and link on the first
	// message where it changes. A stale config from a previous lesson therefore
	// never hijacks a fresh session, and linking needs no /md-log command, no
	// pre-existing note at session start, and no daemon restart. The path may be
	// a Windows path ("C:\\Users\\...\\note.md"); it is translated for WSL.
	// An explicit /md-log or /md-unlog in the session's history still wins.
	const cfgWarned = new Set<string>();
	let cfgFingerprint: string | null = null;
	let lastBackfilledId: string | null = null;

	function readConfigFingerprint(): string | null {
		try {
			const cfgPath = path.join(os.homedir(), ".prime", "agent", "md-log-config.json");
			const st = fs.statSync(cfgPath);
			const content = fs.readFileSync(cfgPath, "utf-8");
			return `${st.mtimeMs}:${st.size}:${content}`;
		} catch {
			return null; // no config file
		}
	}

	function maybeAutoLink(ctx: any): void {
		if (logFile) return;
		const fp = readConfigFingerprint();
		if (fp === cfgFingerprint) return;
		cfgFingerprint = fp;
		if (!fp) return;
		try {
			const cfgPath = path.join(os.homedir(), ".prime", "agent", "md-log-config.json");
			const cfg = JSON.parse(fs.readFileSync(cfgPath, "utf-8"));
			const raw = typeof cfg.file === "string" ? cfg.file.trim() : "";
			if (!raw) return;
			const resolved = normalizeLogPath(raw);
			if (!fs.existsSync(resolved)) {
				if (!cfgWarned.has(resolved)) {
					cfgWarned.add(resolved);
					ctx.ui.notify(`md-log config file missing: ${resolved}`, "error");
				}
				return;
			}
			logFile = resolved;
			...
			ctx.ui.notify(`md-log linked: ${resolved} (${written} entries backfilled)`, "success");
		} catch (err) {
			// Never let config trouble break the session.
			try {
				ctx.ui.notify(`md-log auto-link failed: ${String(err)}`, "error");
			} catch {
				// ignore
			}
		}
	...
	function currentLeafId(ctx: any): string | null {
	...
	function answerCalloutQuiz(details: any): string {
		...
		if (status === "plain_chat") {
			return callout("warning", "Quiz — ask in plain text", [details?.message || ""]);
		}
	...
	function answerCalloutAsk(details: any): string {
		...
		if (status === "plain_chat") {
			return callout("warning", "Question — ask in plain text", [details?.message || ""]);
		}
		const answers: any[] = details?.answers || [];
	...
	pi.on("message_end", async (event, ctx) => {
		if (!logFile) maybeAutoLink(ctx);
		if (!logFile) return;
		// If the link just happened, backfill may already cover this exact
		// message — appending it again would duplicate it in the note.
		if (lastBackfilledId !== null && lastBackfilledId === currentLeafId(ctx)) {
	...
	// Obsidian users paste paths from Windows Explorer ("C:\\Users\\...\\note.md").
	// On Linux (incl. WSL) translate those to the mounted-drive path
	// ("/mnt/c/Users/.../note.md") and normalize any backslashes. On real Windows
	// the paths are already native and are left untouched.
	function normalizeLogPath(input: string): string {
	...
	pi.registerCommand("md-log", {
		...
		handler: async (args, ctx: any) => {
			const filepath = normalizeLogPath(args);
			if (!filepath) {
			...
			if (!fs.existsSync(resolved)) {
				ctx.ui.notify(`File does not exist: ${resolved} — create the note in Obsidian first, then link it.`, "error");
				return;
			}
